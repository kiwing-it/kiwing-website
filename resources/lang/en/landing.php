<?php

return [

    'header' => [
        'home'     => 'Home',
        'ideology' => 'Philosophy',
        'services' => 'Services',
        'about'    => 'Company',
        'work'     => 'Work',
        'team'     => 'Team',
        'contact'  => 'Contact',
    ],

    'home' => [
        'main_message' => 'Knowledge, creativity and passion to maximize your business!',
    ],

    'ideology' => [
        'intro' => [
            'greeting' => 'Hi!',
            'short_presentation' => [
                'not_highlighted' => 'We are an IT Lab specialized in Software Development located in Argentina. We offer top-notch solutions in order to',
                'highlighted'     => 'ensure your business success',
            ],
        ],
        'process' => [
            'title' => 'Our philosophy',
            'values' => [
                'first' => [
                    'title_top_part'    => 'Agile',
                    'title_bottom_part' => 'Processes',
                    'description'       => 'Market demands have changed, and we adapt to it. We believe in flexibility and immediacy of functional high quality results.',
                ],
                'second' => [
                    'title_top_part'    => 'Direct',
                    'title_bottom_part' => 'Communication',
                    'description'       => 'We are social beings and live in constant relationship with others. Maintaining direct and clear communication is one of our main pillars.',
                ],
                'third' => [
                    'title_top_part'    => 'Continuous',
                    'title_bottom_part' => 'Training',
                    'description'       => 'Social, technological and economic changes are taking an exponential form. Being in constant sync with the latest is the most entertaining part.',
                ],
            ],
        ],
        'testimonials' => [
            'title' => 'Testimonials',
            'first' => [
                'content' => 'We\'ve been working with Kiwing for years now, and our experience has always been amazing. They really know their stuff, and the quality of their work is outstanding.',
                'author_name' => 'Tomas O\'Farrell',
                'author_position' => 'Co-Founder at Workana',
            ],
            'second' => [
                'content' => 'Trabajar con Kiwing es una experiencia recomendable. Son un socio confiable, con amplia experiencia llevando adelante desarrollos de aplicaciones web complejas.',
                'author_name' => 'Leandro Masri',
                'author_position' => 'CPO at AnimaEdu',
            ],
            'third' => [
                'content' => 'Phasellus pulvinar lacinia elit nec tristique. Nulla accumsan velit id nulla tincidunt ornare. Sed cursus urna sed. Aliquam elementum orci at dolor viverra.',
                'author_name' => 'One person',
                'author_position' => 'CTO in Facebook',
            ],
        ],
    ],

    'services' => [
        'title' => 'Services',
        'first' => [
            'name' => '1. Web Applications',
            'short_description' => 'We have developed platforms that meet the challenges of the Web: Usability, Scalability, Security, and Stability.',
            'modal' => [
                'title'       => 'Web Applications',
                'description' => '',
                'trigger'     => 'More info',
            ],
        ],
        'second' => [
            'name' => '2. Mobile Applications',
            'short_description' => 'We develop applications for iOS and Android with the best User Experience, Accessibility, and Integration.',
            'modal' => [
                'title'       => 'Mobile Applications',
                'description' => '',
                'trigger'     => 'More info',
            ],
        ],
        'third' => [
            'name' => '3. Full Life-Cycle',
            'short_description' => 'We work with your business experts to capture your vision and then move forward with functional iterations.',
            'modal' => [
                'title'       => 'Full Life-Cycle',
                'description' => '',
                'trigger'     => 'More info',
            ],
        ],
        'fourth' => [
            'name' => '4. Dedicated Teams',
            'short_description' => 'We integrate to remote teams with total flexibility in order to speed it up.',
            'modal' => [
                'title'       => 'Dedicated Teams',
                'description' => '',
                'trigger'     => 'More info',
            ],
        ],
        'fifth' => [
            'name' => '5. Auditory',
            'short_description' => 'We make an in-depth analysis to identify errors and problems in the company core.',
            'modal' => [
                'title'       => 'Auditory',
                'description' => '',
                'trigger'     => 'More info',
            ],
        ],
        'sixth' => [
            'name' => '6. Consultancy',
            'short_description' => 'We have first-rate specialists to provide solutions to your business problems.',
            'modal' => [
                'title'       => 'Consultancy',
                'description' => '',
                'trigger'     => 'More info',
            ],
        ],
    ],

    'about' => [
        'title' => 'Company',
        'boxes' => [
            'first'  => [
                'title'       => 'Philosophy',
                'subtitle'    => 'Passion, Creativity & Knowledge...',
                'description' => 'We have a great passion for what we do, it\'s our lifestyle. For us, the development of high-quality software is a creative process that requires advanced knowledge about the different actors that are involved in it.',
            ],
            'second' => [
                'title'       => 'Who are we?',
                'description' => [
                    'p1' => 'We are a group of engineers and advanced students of Information Systems Engineering. Our orientation towards Computer Science makes us highly qualified and specialized in different fields. Our priority is to satisfy needs through the early and continuous delivery of quality software.',
                    'p2' => 'We believe in technology being the ultimate solution to unlock humans to work more efficiently and effectively.',
                ],
            ],
            'third'  => [
                'title'       => 'Why choose us?',
                'description' => 'Because here you will find a team of excellent developers, but most importantly, great people who have a deep passion for what they do.',
            ],
            'fourth' => [
                'title'       => 'Motto',
                'description' => 'Design, development and innovation to positively change the reality of our customers.',
            ],
        ],
        'skills' => [
            'title'  => 'Skills',
            'first'  => 'Backend Development',
            'second' => 'Frontend Development',
            'third'  => 'Mobile Development',
            'fourth' => 'Analysis and Design',
            'fifth'  => 'Architecture',
        ],
    ],

    'ourcount' => [
        'title' => 'Significant numbers',
        'subtitle' => 'Here we present some accounts that highlight our trajectory in a synthetic way.',
        'items' => [
            'first'  => 'International products',
            'second' => 'Client rehirements',
            'third'  => 'Completed projects',
            'fourth' => 'Lines of code',
        ],
    ],

    'work' => [
        'title' => 'Recent work',
        'subtitle' => 'Here are some of the latest projects we have been working on.',
        'categories' => [
            'all' => 'All',
            'web_application'     => 'Web Application',
            'mobile_application'  => 'Mobile Application',
            'progressive_web_app' => 'Progressive Web Application',
        ],
        'items' => [
            'workana' => [
                'title' => 'Workana',
                'description' => 'The first and largest Latin American freelance marketplace.',
            ],
            'pausa' => [
                'title' => 'Pausa',
                'description' => 'Pausa is the most complete app to learn to meditate and incorporate mindfulness into your life.'
            ],
            'animaedu' => [
                'title' => 'AnimaEdu',
                'description' => 'AnimaEdu is a marketplace of video courses for personal development.',
            ],
            'family_tree' => [
                'title' => 'Family Trees App',
                'description' => 'Family trees app with inference of non-trivial kinship relationships, mergers and combinations of trees, or logical queries on large numbers of nodes. We have used a graph-oriented database called Neo4j.',
            ],
        ],
    ],

    'contact' => [
        'title'    => 'Contact',
        'subtitle' => 'Drop us a line and we will contact you as soon as possible.',
        'form' => [
            'title'       => 'Contact us',
            'subtitle'    => 'We will provide you with personalized attention',
            'description' => 'One of our consultants will contact you to listen to your needs and help you find the best solution for your business.',
        ],
        'placeholder' => [
            'subject' => 'Subject *',
            'phone'   => 'Phone',
            'name'    => 'Name *',
            'email'   => 'Email *',
            'message' => 'Message *',
        ],
        'validation' => [
            'required' => 'Required.',
        ],
        'button' => [
            'send' => 'Send',
        ],
    ],

    'footer' => [
        'up' => 'Up',
        'all_rights_reserved' => '&copy; 2018 Kiwing IT Labs. All rights reserved.',
    ],

    'seo' => [
        'title' => 'Kiwing | Top-notch Software Development - Argentina',
        'description' => 'We are an IT Lab specialized in Software Development located in Argentina. We offer top-notch solutions in order to ensure your business success.',
        'keywords' => 'argentina, agile, agile processes, direct communication, continuous training, top-notch solutions, software development, mobile, app, api, web application, progressive web application, pwa, laravel, php, javascript, vue, react, react native, node, ios, android, web, ionic, mysql, sql, software architecture, design patterns, passion, creativity, knowledge, full life-cycle, dedicated teams',
    ],
    
];
